...
test.describe('EC2 - VPC Tabs Navigation', () => {
  test('navigate to EC2 page and verify VPC tabs visible', async ({ page }) => {
    await page.goto('/#/services/ec2')
    await page.waitForLoadState('networkidle')
  ...
  test('click each VPC tab and verify tab is selectable', async ({ page }) => {
    await page.goto('/#/services/ec2')
    await page.waitForLoadState('networkidle')
...
test.describe('EC2 - VPC Tab Create Dialogs', () => {
  test('VPCs tab — Create VPC dialog opens and cancels', async ({ page }) => {
    await page.goto('/#/services/ec2')
    await page.waitForLoadState('networkidle')
    await page.getByRole('tab', { name: 'VPCs' }).click()
    await page.waitForTimeout(500)
  ...
  test('VPCs tab — Create VPC form has CidrBlock field', async ({ page }) => {
    await page.goto('/#/services/ec2')
    await page.waitForLoadState('networkidle')
    await page.getByRole('tab', { name: 'VPCs' }).click()
    await page.waitForTimeout(500)
  ...
  test('Subnets tab — Create Subnet dialog opens and cancels', async ({ page }) => {
    await page.goto('/#/services/ec2')
    await page.waitForLoadState('networkidle')
  ...
  test('Subnets tab — Create Subnet form has form fields', async ({ page }) => {
    await page.goto('/#/services/ec2')
    await page.waitForLoadState('networkidle')
    await page.getByRole('tab', { name: 'Subnets' }).click()
    await page.waitForTimeout(500)
  ...
  test('Route Tables tab — Create Route Table dialog opens and cancels', async ({ page }) => {
    await page.goto('/#/services/ec2')
    await page.waitForLoadState('networkidle')
  ...
  test('Internet GWs tab — Create IGW dialog opens and cancels', async ({ page }) => {
    await page.goto('/#/services/ec2')
    await page.waitForLoadState('networkidle')
  ...
  test('NAT Gateways tab — Create NAT Gateway dialog opens and cancels', async ({ page }) => {
    await page.goto('/#/services/ec2')
    await page.waitForLoadState('networkidle')
  ...
  test('Network ACLs tab — Create Network ACL dialog opens and cancels', async ({ page }) => {
    await page.goto('/#/services/ec2')
    await page.waitForLoadState('networkidle')
  ...
  // Flow Logs tab hidden on emulator — test removed since tab no longer visible
  // test('Flow Logs tab — shows unsupported message when emulator does not support Flow Logs', async ({ page }) => {
  //   await page.goto('/#/services/ec2')
  //   await page.waitForLoadState('networkidle')
  //   await page.getByRole('tab', { name: 'Flow Logs' }).click()
  //   await page.waitForTimeout(1000)
  //   await expect(page.getByRole('heading', { name: 'VPC Flow Logs Not Available' })).toBeVisible({ timeout: 10000 })
  //   await expect(page.getByText('VPC Flow Logs are not supported by your AWS emulator')).toBeVisible({ timeout: 5000 })
  // })
})

test.describe('EC2 - VPC Pagination', () => {
  test('VPCs tab pagination controls present', async ({ page }) => {
    await page.goto('/#/services/ec2')
    await page.waitForLoadState('networkidle')
    await page.getByRole('tab', { name: 'VPCs' }).click()
    await page.waitForTimeout(1000)
    // Pagination or per-page selector should be present when items exist
    const hasPagination = await page.getByText('Show:').first().isVisible({ timeout: 2000 }).catch(() => false)
    if (hasPagination) {
      await expect(page.getByText('Show:').first()).toBeVisible()
      await expect(page.getByText('per page').first()).toBeVisible()
    } else {
      // Empty state shown — still valid
      await expect(page.getByText(/no|empty|vpc/i).first()).toBeVisible({ timeout: 5000 }).catch(() => {})
    }
  })
})

test.describe('EC2 - VPC Usage Examples', () => {
  test('VPC CLI commands shown in usage examples', async ({ page }) => {
    await page.goto('/#/services/ec2')
    await page.waitForLoadState('networkidle')
    // Scroll to Usage Examples section
    const usageHeading = page.getByRole('heading', { name: 'Usage Examples' })
    await expect(usageHeading).toBeVisible({ timeout: 10000 })
    await usageHeading.scrollIntoViewIfNeeded()
    await page.waitForTimeout(500)
    // Check AWS CLI tab has VPC commands
    const codeBlock = page.locator('pre').filter({ hasText: /aws ec2.*vpc|create-vpc|create-subnet/ })
    await expect(codeBlock).toBeVisible({ timeout: 10000 })
  })
})

test.describe('EC2 - Elastic IPs Tab', () => {
  test('Elastic IPs tab renders and shows empty state', async ({ page }) => {
    await page.goto('/#/services/ec2')
    await page.waitForLoadState('networkidle')
  ...
  test('Elastic IPs tab — Allocate button opens no error', async ({ page }) => {
    await page.goto('/#/services/ec2')
    await page.waitForLoadState('networkidle')
    await page.getByRole('tab', { name: 'Elastic IPs' }).click()
    await page.waitForTimeout(500)
    // Verify the allocate button or empty state action exists
