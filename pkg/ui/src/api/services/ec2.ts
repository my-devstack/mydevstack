...
import type { EC2Instance, EC2KeyPair, EC2SecurityGroup, EC2Vpc, EC2Subnet } from '@/api/types/aws'
...
export async function deleteKeyPair(keyName: string): Promise<void> {
  await request<{}>('DELETE', `/key-pairs/${encodeURIComponent(keyName)}`)
}
...
export async function deleteSecurityGroup(groupId: string): Promise<void> {
  await request<{}>('DELETE', `/security-groups/${encodeURIComponent(groupId)}`)
}
...
export async function authorizeSecurityGroupIngress(
  ...
  await request<{}>('POST', `/security-groups/${encodeURIComponent(groupId)}/ingress`, params)
}
