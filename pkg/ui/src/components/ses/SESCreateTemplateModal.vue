...
const emit = defineEmits<{
  ...
  'create': [name: string, subject: string, htmlBody: string, textBody: string]
}>()

const form = defineModel<{ name: string; subject: string; htmlBody: string; textBody: string }>('form', {
  default: { name: '', subject: '', htmlBody: '', textBody: '' },
})
  ...
  >
    ...
    <template #footer>
      <div class="flex justify-end gap-2">
        ...
        <Button @click="emit('create', form.name, form.subject, form.htmlBody, form.textBody)">
          Create
        </Button>
      </div>
    </template>
