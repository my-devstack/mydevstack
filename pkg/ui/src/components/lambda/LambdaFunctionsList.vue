  ...
  >
    ...
    >
      ...
      >
        ...
        <div class="w-20 flex-shrink-0 text-light-muted dark:text-dark-muted">
        ...
        >
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
              ...
              <textarea
                :value="getPayload(func.FunctionName)"
                class="w-full px-3 py-2 text-sm rounded-lg border bg-light-surface dark:bg-dark-surface border-light-border dark:border-dark-border text-light-text dark:text-dark-text font-mono"
                rows="3"
                placeholder="{&quot;key&quot;: &quot;value&quot;}"
                @input="setPayload(func.FunctionName, ($event.target as HTMLTextAreaElement).value)"
              />
            </div>
            <div>
              <label class="block text-xs font-medium text-light-muted dark:text-dark-muted uppercase mb-1">Invocation Type</label>
              <select
                :value="getInvocationType(func.FunctionName)"
                class="w-full px-3 py-2 text-sm rounded-lg border bg-light-surface dark:bg-dark-surface border-light-border dark:border-dark-border text-light-text dark:text-dark-text"
                @change="setInvocationType(func.FunctionName, ($event.target as HTMLSelectElement).value)"
              >
                <option
                  v-for="type in invocationTypes"
                  :key="type.value"
                  :value="type.value"
                >
                  {{ type.label }}
          ...
          <div class="flex gap-2 mb-4">
            <button
              ...
              :disabled="isLoading(func.FunctionName)"
              @click.stop="handleInvoke(func.FunctionName)"
            >
          ...
          <div v-if="getResult(func.FunctionName)">
            ...
            <pre
              class="p-3 rounded-lg overflow-auto max-h-40 text-sm font-mono"
